    ...
    <Highlight {...defaultProps} code={codeString} language={language} theme={theme}>
      {({ tokens, getLineProps, getTokenProps }) => (
        <div className="relative" onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)}>
          {tokens.length > 2 ? (
            <div
              className={`absolute top-10 right-4 h-8 w-8 cursor-pointer rounded border border-gray-200 p-1 shadow transition-opacity ${show ? 'opacity-100' : 'opacity-0'
                }`}
              onClick={() => {
                copy(codeString);
                setCopied(true);
          ...
          <pre className="bg-gray-900 dark:bg-gray-700">
            {tokens.map((line, i) => {
              return (
                ...
                >
                  <span className="mr-4 inline-block w-6 select-none text-right text-gray-400">{i + 1}</span>
                  {line.map((token, key) => {
            ...
            <div className="border-l-4 border-gray-900 bg-gray-900">
              <span className="bg-brand-dark ml-10 mr-4 inline-block px-2">Language {language}</span>
              {fileName ? <span className="bg-brand-dark mr-4 inline-block px-2">{fileName}</span> : ''}
              {url ? (
                <span className="mr-4 inline-block bg-white px-2">
